...
func (*AdminService) Initialize(e echo.Context) error {
	...
	host := util.GetEnv("BENCHMARK_SERVER_HOST", "10.162.47.101")
...
func (*ContestantService) Dashboard(e echo.Context) error {
	...
	res, err, _ := sfGroup.Do(fmt.Sprintf("dashboard%d", team.ID), func() (interface{}, error) {
                        ...
                        //end := time.Now()
			//log.Printf("team:%d %f秒\n", team.ID, (end.Sub(start)).Seconds())
			if err != nil {
...
func (*RegistrationService) CreateTeam(e echo.Context) error {
	...
	_, err = conn.ExecContext(ctx, "LOCK TABLES `teams` WRITE, `contestants` WRITE, `team_scores` WRITE")
	if err != nil {
...
func (*RegistrationService) UpdateRegistration(e echo.Context) error {
	...
	_, err = tx.Exec(
		"UPDATE team_scores LEFT JOIN (SELECT team_id,(SUM(`student`) = COUNT(*)) AS `student` FROM `contestants` WHERE `contestants`.`team_id` = ?) `team_student_flags` ON `team_scores`.`team_id` = `team_student_flags`.`team_id` SET `team_scores`.`name`=?, `team_scores`.`student_flag` = `team_student_flags`.`student` WHERE `team_scores`.`team_id` = ?",
		team.ID,
		req.TeamName,
		team.ID,
	)
...
func (*RegistrationService) DeleteRegistration(e echo.Context) error {
	...
	_, err = tx.Exec(
		"UPDATE team_scores LEFT JOIN (SELECT team_id,(SUM(`student`) = COUNT(*)) AS `student` FROM `contestants` WHERE `contestants`.`team_id` = ?) `team_student_flags` ON `team_scores`.`team_id` = `team_student_flags`.`team_id` SET `team_scores`.`withdrawn`=TRUE,`team_scores`.`leader_id`=NULL, `team_scores`.`student_flag` = `team_student_flags`.`student` WHERE `team_scores`.`team_id` = ?",
	)
	if err != nil {
		return fmt.Errorf("update team: %w", err)
	}
		...
		case <-ticker.C:
			...
			if err == nil {
				...
				if contestFreezesAt.Before(n) && !contestFinished {
					audienceDashboardCacheTime = n.UnixNano() + 80000000000 // contestStatus.ContestEndsAt.UnixNano() // n.UnixNano() + 800000000
				} else {
